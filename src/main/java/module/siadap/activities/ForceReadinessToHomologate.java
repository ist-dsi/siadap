...
public class ForceReadinessToHomologate extends WorkflowActivity<SiadapProcess, ActivityInformation<SiadapProcess>> {
	...
	public boolean isActive(SiadapProcess process, User user) {
		...
		return  siadapYearConfiguration.getCcaMembers().contains(user.getPerson()) && state.ordinal() >= SiadapProcessStateEnum.WAITING_SUBMITTAL_BY_EVALUATOR_AFTER_VALIDATION.ordinal() && state.ordinal() < SiadapProcessStateEnum.WAITING_HOMOLOGATION.ordinal(); 
	}
