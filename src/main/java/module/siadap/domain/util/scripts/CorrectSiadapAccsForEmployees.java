...
public class CorrectSiadapAccsForEmployees extends WriteCustomTask {
    ...
    private static final Integer YEAR_TO_CORRECT = 2013;
    ...
    protected void doService() {
        final LocalDate dateToUse = SiadapMiscUtilClass.lastDayOfYearWhereAccsAreActive(YEAR_TO_CORRECT);
        //all of the relevant acc types
        List<AccountabilityType> relevantAccountabilityTypes = new ArrayList<AccountabilityType>();
        SiadapYearConfiguration siadapYearConfiguration = SiadapYearConfiguration.getSiadapYearConfiguration(YEAR_TO_CORRECT);
        ...
        Map<Person, Map<AccountabilityType, List<Accountability>>> duplicatedAccsForPerson =
                new HashMap<Person, Map<AccountabilityType, List<Accountability>>>();
...
//        }
        ...
        out.println("\n\n\nDeleting the duplicated work accs");
        for (final Person person : personsWithDuplicatedWorkAccs) {
            ...
            final Collection<Accountability> parentAccountabilities =
                    person.getParentAccountabilities(siadapYearConfiguration.getWorkingRelation(),
                            siadapYearConfiguration.getWorkingRelationWithNoQuota());
            HashSet<Accountability> filteredSet =
                    Sets.newHashSet(Iterables.filter(parentAccountabilities, new Predicate<Accountability>() {
