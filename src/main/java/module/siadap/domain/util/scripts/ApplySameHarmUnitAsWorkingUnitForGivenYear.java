...
public class ApplySameHarmUnitAsWorkingUnitForGivenYear extends ReadCustomTask {
	...
	static final Multiset<Siadap> siadapsWithDifferentUniverses = HashMultiset
			.create();
			...
			for (String siadapId : siadaps) {
				...
				if (siadap.getYear() == 2012) {
					try {
						...
						PersonSiadapWrapper previousSiadap = new PersonSiadapWrapper(
								siadap.getEvaluated(), 2011);
						if (previousSiadap.getSiadap() != null
								&& !siadap.getDefaultSiadapUniverse().equals(
										previousSiadap
												.getDefaultSiadapUniverse())) {
							if (siadap.getState().ordinal() > SiadapProcessStateEnum.NOT_CREATED
									.ordinal()
									&& previousSiadap.getSiadap().getState()
											.ordinal() > SiadapProcessStateEnum.NOT_CREATED
											.ordinal())
				...
				PersonSiadapWrapper personSiadapWrapper = new PersonSiadapWrapper(
						siadap);
				SiadapUniverse defaultSiadapUniverse = personSiadapWrapper
						.getDefaultSiadapUniverse();
				Unit unitWhereIsHarmonized = null;
				try {

					unitWhereIsHarmonized = personSiadapWrapper
							.getUnitWhereIsHarmonized(defaultSiadapUniverse);
				} catch (SiadapException ex) {
					out.println("This fella has more than 1 harmonization unit!! siadap id: "
							+ siadap.getExternalId());

				}
				UnitSiadapWrapper workingUnit = personSiadapWrapper
						.getWorkingUnit();

				if (personSiadapWrapper.getPerson() == null) {
					out.println("dafuq?! got a process without a person!! Siadap externalID: "
							+ siadap.getExternalId());
				} else

				if (workingUnit == null || workingUnit.getUnit() == null) {
					out.println("Unidade de trabalho vazia |\t "
							+ personSiadapWrapper.getPerson()
									.getPresentationName()
				...
				if (unitWhereIsHarmonized == null) {
					SiadapUniverse previousSiadapUniverse = new PersonSiadapWrapper(
							siadap.getEvaluated(), Integer.valueOf((siadap
									.getYear().intValue() - 1)))
							.getDefaultSiadapUniverse();

					if (previousSiadapUniverse != null
							&& !previousSiadapUniverse
									.equals(defaultSiadapUniverse)
							&& personSiadapWrapper
									.getUnitWhereIsHarmonized(previousSiadapUniverse) != null) {
						...
						out.println("Mudança de universo |\t "
								+ personSiadapWrapper.getPerson()
										.getPresentationName() + " |\t"
								+ workingUnit.getUnit().getPresentationName()
								+ "|\t não definida |\t "
								+ unitWhereIsHarmonized.getPresentationName()
								+ " |\t " + siadap.getYear() + " |\t"
								+ siadap.getState().getLocalizedName());
						if (!DRY_RUN) {
							//TODO
					...
					try {
						...
						Unit newHarmonizationUnit = workingUnit
								.getHarmonizationUnit();

						if (!unitWhereIsHarmonized.equals(newHarmonizationUnit)) {
							String newHarmonizationUnitString;
							if (newHarmonizationUnit == null) {
								newHarmonizationUnitString = "NULL";

							} else {
								newHarmonizationUnitString = newHarmonizationUnit
										.getPresentationName();
							}
							out.print("Incoerência entre U.T e U.H |\t "
									...
									+ " |\t "
									+ unitWhereIsHarmonized
											.getPresentationName() + " |\t "
									+ newHarmonizationUnitString + " |\t "
									+ siadap.getYear() + " |\t"
									+ siadap.getState().getLocalizedName());
							...
							{
								try {
									personSiadapWrapper.changeHarmonizationUnitTo(workingUnit.getUnit(), DATE_OF_CHANGES, "Correcção automática das unidades de harmonização para reflectirem as unidades de trabalho");
	...
	public void doIt() {
		...
		for (Siadap siadap : SiadapYearConfiguration
				.getSiadapYearConfiguration(year).getSiadaps()) {
			totalSiadaps++;
			siadaps.add(siadap.getExternalId());

		}
		// let's reconnect the disconnected ones
		CorrectHarmRelationsForGivenYear siadapReconnecter = new CorrectHarmRelationsForGivenYear(
				out);
		...
		out.println("Got "
				+ siadapsWithDifferentUniverses.size()
		...
		for (Siadap siadap : siadapsWithDifferentUniverses) {
			out.println(siadap.getEvaluated().getPresentationName()
					+ " ESTADO: " + siadap.getState().getLocalizedName());
