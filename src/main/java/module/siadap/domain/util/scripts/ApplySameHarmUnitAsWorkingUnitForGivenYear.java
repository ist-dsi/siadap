...
public class ApplySameHarmUnitAsWorkingUnitForGivenYear extends ReadCustomTask {
			...
			for (String siadapId : siadaps) {
				...
				if (personSiadapWrapper.getPerson() == null) {
					...
					if (workingUnit == null || workingUnit.getUnit() == null) {
						out.println("Unidade de trabalho vazia |\t " + personSiadapWrapper.getPerson().getPresentationName()
								+ " |\t não definida |\t não definida |\t não definida |\t " + siadap.getYear() + " |\t"
								+ siadap.getState().getLocalizedName());

					} else if (unitWhereIsHarmonized == null) {
						SiadapUniverse previousSiadapUniverse =
								new PersonSiadapWrapper(siadap.getEvaluated(), Integer.valueOf((siadap.getYear().intValue() - 1)))
						...
						if (previousSiadapUniverse != null && !previousSiadapUniverse.equals(defaultSiadapUniverse)
							...
							unitWhereIsHarmonized = personSiadapWrapper.getUnitWhereIsHarmonized(previousSiadapUniverse);
							out.println("Mudança de universo |\t " + personSiadapWrapper.getPerson().getPresentationName()
							...
							try {
								...
								if (unitWhereIsHarmonized == null || !unitWhereIsHarmonized.equals(newHarmonizationUnit)) {
									...
									String unitWhereIsHarmonizedString =
											unitWhereIsHarmonized == null ? "-" : unitWhereIsHarmonized.getPresentationName();
									out.print("Incoerência entre U.T e U.H |\t "
											+ personSiadapWrapper.getPerson().getPresentationName() + " |\t "
											+ workingUnit.getPresentationName() + " |\t " + unitWhereIsHarmonizedString + " |\t "
	...
	public void doIt() {
		...
		for (Siadap siadap : siadapsWithDifferentUniverses) {
			out.println(siadap.getEvaluated().getPresentationName() + " ESTADO: " + siadap.getState().getLocalizedName());
