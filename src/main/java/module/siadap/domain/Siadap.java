...
public class Siadap extends Siadap_Base {
	...
	public static final Comparator<Siadap> COMPARATOR_BY_EVALUATED_PRESENTATION_NAME_FALLBACK_YEAR_THEN_OID = new Comparator<Siadap>() {
		...
		public int compare(Siadap o1, Siadap o2) {
			...
			if (o1.getEvaluated() != null && o2.getEvaluated() != null)
				presentationNameComparison = o1.getEvaluated()
						.getPresentationName()
						.compareTo(o2.getEvaluated().getPresentationName());
			int yearComparison = o1.getYear().compareTo(o2.getYear());
			return presentationNameComparison == 0 ? (yearComparison == 0 ? o1
	...
	static {
		...
		UnreadCommentsWidget.register(new WorkflowCommentCounter(
				SiadapProcess.class));
	}
	...
	public Siadap(int year, Person evaluated, SiadapUniverse siadapUniverse,
		...
		SiadapYearConfiguration.getSiadapYearConfiguration(getYear())
				.addSiadaps(this);
		SiadapEvaluationUniverse siadapEvaluationUniverse = new SiadapEvaluationUniverse(
				this, siadapUniverse, competenceType, true);
	}
	...
	public SiadapProcessStateEnum getState() {
		if (getNulled() == Boolean.TRUE) {
			...
			return SiadapProcessStateEnum.WAITING_VALIDATION;
				...
				&& !getForcedReadinessToHomologation()) {
		...
		LocalDate limitDate = getAcknowledgeValidationDate()
				.plusDays(
						getSiadapYearConfiguration()
								.getReviewCommissionWaitingPeriod());
	...
	public boolean isOngoing() {
		return getState().equals(
				...
				|| getState().equals(
						SiadapProcessStateEnum.WAITING_EVAL_OBJ_ACK)
				|| getState().equals(
						SiadapProcessStateEnum.WAITING_SELF_EVALUATION)
				|| getState().equals(SiadapProcessStateEnum.NOT_YET_EVALUATED)
				|| getState().equals(
						SiadapProcessStateEnum.WAITING_HARMONIZATION)
				|| getState().equals(SiadapProcessStateEnum.WAITING_VALIDATION)
				|| getState()
						.equals(SiadapProcessStateEnum.WAITING_SUBMITTAL_BY_EVALUATOR_AFTER_VALIDATION)
				|| getState()
						.equals(SiadapProcessStateEnum.WAITING_VALIDATION_ACKNOWLEDGMENT_BY_EVALUATED)
				|| getState().equals(
						SiadapProcessStateEnum.VALIDATION_ACKNOWLEDGED);
	}

	public boolean isWaitingForReviewCommission() {
		return getState().equals(
				SiadapProcessStateEnum.WAITING_FOR_REVIEW_COMMISSION);
	}
	...
	public SiadapEvaluationUniverse getDefaultSiadapEvaluationUniverse() {
		for (SiadapEvaluationUniverse evaluationUniverse : getSiadapEvaluationUniverses()) {
			final Boolean defaultEvaluationUniverse = evaluationUniverse
					.getDefaultEvaluationUniverse();
			if (defaultEvaluationUniverse != null
					&& defaultEvaluationUniverse.booleanValue())
	...
	public List<SiadapEvaluationItem> getCurrentEvaluationItems() {
		...
		currentEvaluationItems.addAll(evalUniverse.getEvaluations(
				SiadapEvaluationItem.class, new Predicate() {
	 ...
	 */
	public void setCustomSchedule(
			SiadapProcessSchedulesEnum processSchedulesEnum, LocalDate newDate) {
	...
	public boolean isEvaluatedWithKnowledgeOfObjectives() {
		if (getCurrentEvaluationItems() == null
				|| getCurrentEvaluationItems().isEmpty())
			return false;
	...
	public boolean hasAnyCompetencesSet() {
		if (!getDefaultSiadapEvaluationUniverse().hasAnySiadapEvaluationItems())
			return false;
		ArrayList<SiadapEvaluationItem> evaluationItems = new ArrayList<SiadapEvaluationItem>(
				getSiadapEvaluationItems2());
	...
	public CompetenceType getDefaultCompetenceType() {
		if (getDefaultSiadapEvaluationUniverse() == null)
			return null;
		return getDefaultSiadapEvaluationUniverse()
				.getCompetenceSlashCareerType();
	}

	@Service
	public void createCurricularPonderation(SiadapUniverse siadapUniverse,
			BigDecimal gradeToAssign, Boolean assignedExcellency,
		...
		if (siadapUniverse == null
				|| assignedExcellency == null
				|| evaluator == null
				|| !SiadapGlobalEvaluation.isValidGrade(gradeToAssign,
						assignedExcellency.booleanValue())
				|| (assignedExcellency.booleanValue() && StringUtils
						.isEmpty(excellencyAwardJustification))
				|| StringUtils.isEmpty(curricularPonderationJustification))
			throw new SiadapException(
					"invalid.data.for.creation.of.a.curricular.ponderation");
		...
		if (getSiadapEvaluationUniverseForSiadapUniverse(siadapUniverse) != null)
			throw new SiadapException(
					"error.curricular.ponderation.cannot.have.more.than.one.eval.for.universe");
		...
		Unit siadapSpecialHarmonizationUnit = siadapYearConfiguration
				.getSiadapSpecialHarmonizationUnit();
		if (siadapSpecialHarmonizationUnit == null) {
			throw new SiadapException(
					"error.must.configure.special.harmonnization.unit.first");
		}

		AccountabilityType accTypeToReplace = null;
		if (siadapUniverse.equals(SiadapUniverse.SIADAP2)) {
			accTypeToReplace = siadapYearConfiguration
		...
		for (Accountability accountability : evaluated
				.getParentAccountabilities(accTypeToReplace)) {
			...
			if (accountability.getParent() instanceof Unit) {
				if (accountability.isActive(SiadapMiscUtilClass
						.lastDayOfYear(getYear()))) {
					...
					throw new SiadapException(
							"already.with.a.curricular.ponderation.attributed");
				}
			}
		}
		...
		if (accToRemove != null)
			accToRemove.delete();
		evaluated.addParent(siadapSpecialHarmonizationUnit, accTypeToReplace,
				dateToUse, SiadapMiscUtilClass.lastDayOfYear(getYear()));
	...
	public ArrayList<Competence> getCompetences() {
		...
		for (SiadapEvaluationItem siadapEvaluationItem : evaluationItems) {
			if (siadapEvaluationItem instanceof CompetenceEvaluation)
				arrayCompetences
						.add(((CompetenceEvaluation) siadapEvaluationItem)
								.getCompetence());
		}
		return arrayCompetences;
	}
	...
	public boolean isWithObjectivesFilled() {
		...
		Integer currentObjectiveVersion = getDefaultSiadapEvaluationUniverse()
				.getCurrentObjectiveVersion();
		...
		for (SiadapEvaluationItem item : getSiadapEvaluationItems2()) {
			...
			if (item instanceof CompetenceEvaluation) {
				...
				if (objectiveEvaluation
						.isValidForVersion(currentObjectiveVersion)) {
	...
	public boolean hasAllNeededCompetences() {
		return (getEvaluatedOnlyByCompetences() != null && ((getEvaluatedOnlyByCompetences()
				.booleanValue() == false && getCompetences().size() >= MINIMUM_COMPETENCES_WITH_OBJ_EVAL_NUMBER) || (getEvaluatedOnlyByCompetences()
				.booleanValue() == true && getCompetences().size() >= MINIMUM_COMPETENCES_WITHOUT_OBJ_EVAL_NUMBER)));
	...
	public SiadapEvaluationUniverse getSiadapEvaluationUniverseForSiadapUniverse(
		...
		for (SiadapEvaluationUniverse siadapEvaluationUniverse : getSiadapEvaluationUniverses()) {
			if (siadapEvaluationUniverse.getSiadapUniverse() != null
					&& siadapEvaluationUniverse.getSiadapUniverse().equals(
							siadapUniverse))
	...
	public SiadapGlobalEvaluation getSiadapGlobalEvaluationEnum(
			SiadapUniverse siadapUniverse, boolean considerValidation) {
		return getSiadapEvaluationUniverseForSiadapUniverse(siadapUniverse)
				.getSiadapGlobalEvaluationEnum(considerValidation, false);
	 ...
	 */
	public boolean hasGivenSiadapGlobalEvaluation(
			SiadapGlobalEvaluation siadapGlobalEvaluation,
			SiadapUniverse siadapUniverseToConsider) {
		return hasGivenSiadapGlobalEvaluation(siadapGlobalEvaluation,
				siadapUniverseToConsider, false);
	}
	...
	public boolean hasGivenSiadapGlobalEvaluation(
		...
		if (siadapEvaluationUniverseForSiadapUniverse == null
				&& siadapGlobalEvaluation
						.equals(SiadapGlobalEvaluation.NONEXISTING)) {
			return true;
		}
		if (siadapEvaluationUniverseForSiadapUniverse == null)
			return false;
		...
		if (siadapEvaluationUniverseForSiadapUniverse
				.getDefaultEvaluationUniverse() && !isDefaultEvaluationDone())

		{
			if (siadapGlobalEvaluation
					.equals(SiadapGlobalEvaluation.NONEXISTING))
				return true;
			else
				return false;
		...
		if (considerValidation
				&& siadapEvaluationUniverseForSiadapUniverse
						.hasValidationAssessment()) {
			gradeToUse = siadapEvaluationUniverseForSiadapUniverse
					.getCcaClassification();
			excellencyAward = siadapEvaluationUniverseForSiadapUniverse
	...
	public boolean hasExcellencyAward() {
		if (getEvaluationData2() == null
				|| getEvaluationData2().getExcellencyAward() == null)
			return false;
	...
	public void markAsHarmonized(LocalDate harmonizationDate,
		...
		if ((evaluationUniverse.getHarmonizationAssessment() == null && !evaluationUniverse
			...
			throw new SiadapException(
					"harmonization.error.there.are.people.not.harmonized");
		...
		if (!isEvaluationDone(siadapUniverse)) {
			if (evaluationUniverse.getDefaultEvaluationUniverse()
					&& isWithSkippedEvaluation()) {
				// do nothing :)
			} else {
				throw new SiadapException(
						"error.harmonization.can't.harmonize.with.users.without.grade");
			}
		}
		...
		if (evaluationUniverse.getHarmonizationAssessment() != null
				&& evaluationUniverse
						.getHarmonizationAssessmentForExcellencyAward() != null
				&& evaluationUniverse
						.getHarmonizationAssessmentForExcellencyAward() == true
	...
	public boolean isSuggestedForExcellencyAward() {
		...
		return evaluationData != null
				&& evaluationData.getExcellencyAward() == Boolean.TRUE;
	}

	public boolean isWithSkippedEvaluation() {
		SiadapEvaluation evaluationData = getEvaluationData2();
		return evaluationData != null
				&& !StringUtils.isEmpty(evaluationData
						.getNoEvaluationJustification());
	}
	...
	public boolean hasAllEvaluationItemsValid() {
		for (SiadapEvaluationUniverse evaluationUniverse : getSiadapEvaluationUniverses()) {
			for (ObjectiveEvaluation objectiveEvaluation : evaluationUniverse
	...
	public boolean isCoherentOnTypeOfEvaluation() {
		...
		if ((getEvaluatedOnlyByCompetences() != null && getEvaluatedOnlyByCompetences())
				...
				&& defaultSiadapEvaluationUniverse.getObjectiveEvaluations()
						.size() != 0)
			return false;
		return true;
	}
	...
	public boolean validateExistenceOfOnlyOneSetOfEvaluationAndAutoEvaluation() {
		boolean foundOneSet = false;
		for (SiadapEvaluationUniverse evaluationUniverse : getSiadapEvaluationUniverses()) {
			SiadapEvaluation siadapEvaluation = evaluationUniverse
					.getSiadapEvaluation();
			SiadapAutoEvaluation siadapAutoEvaluation = evaluationUniverse
					.getSiadapAutoEvaluation();
	...
	public List<SiadapEvaluationItem> getSiadapEvaluationItems2() {
		List<SiadapEvaluationItem> siadapEvaluationItems = getDefaultSiadapEvaluationUniverse()
				.getSiadapEvaluationItems();
		if (siadapEvaluationItems != null && siadapEvaluationItems.size() > 0)
	...
	public void setDefaultSiadapUniverse(SiadapUniverse siadapUniverseToChangeTo) {
		...
		if (getSiadapEvaluationUniverseForSiadapUniverse(siadapUniverseToChangeTo) != null
				&& !getSiadapEvaluationUniverseForSiadapUniverse(
						siadapUniverseToChangeTo).equals(
						getDefaultSiadapEvaluationUniverse())) {
			throw new SiadapException(
					"error.cant.change.default.universe.because.theres.another.eval.in.that.universe");
		}
		getDefaultSiadapEvaluationUniverse().setSiadapUniverse(
	...
	public static SiadapUniverse getLastSiadapUniverseUsedBy(Person person) {
		...
		for (Siadap previousSiadap : person.getSiadapsAsEvaluated()) {
			SiadapUniverse defaultSiadapUniverse = previousSiadap
					.getDefaultSiadapUniverse();
			if (defaultSiadapUniverse != null
					&& (mostRecentYear == 0 || previousSiadap.getYear() < mostRecentYear)) {
				siadapUniverseToReturn = previousSiadap
						.getDefaultSiadapUniverse();
			}
		}
	...
	public static String getRemoteEmail(Person person) {
		return new JerseyRemoteUser(person.getUser())
