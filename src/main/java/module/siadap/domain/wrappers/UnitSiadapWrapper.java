...
public class UnitSiadapWrapper extends PartyWrapper implements Serializable {
	...
	public static final Comparator<UnitSiadapWrapper> COMPARATOR_BY_UNIT_NAME = new Comparator<UnitSiadapWrapper>() {
		...
		public int compare(UnitSiadapWrapper o1, UnitSiadapWrapper o2) {
			return o1.getName().compareTo(o2.getName()) != 0 ? o1.getName()
					.compareTo(o2.getName()) : o1.getUnit().getExternalId()
					.compareTo(o2.getUnit().getExternalId());
		}
	};
	...
	public Set<SiadapUniverseWrapper> getAllUniverseWrappersOfAllPeopleInSubUnits() {
		...
		SiadapUniverseWrapper peopleWithQuotasSIADAP2 = new SiadapUniverseWrapper(
				getSiadap2AndWorkingRelationWithQuotaUniverse(),
		...
		SiadapUniverseWrapper peopleWithQuotasSIADAP3 = new SiadapUniverseWrapper(
				getSiadap3AndWorkingRelationWithQuotaUniverse(),
	...
	public Collection<SiadapUniverseWrapper> getValidationUniverseWrappers() {
		...
		ExceedingQuotaProposal.organizeAndFillExceedingQuotaProposals(
				getUnit(), getYear(), siadap2WithQuotas, siadap3WithoutQuotas,
				siadap2WithoutQuotas, siadap3WithQuotas);
		...
		universeWrappers.add(new SiadapUniverseWrapper(
				validationSiadap2WithQuotas.values().iterator().next(),
				SiadapUniverseWrapper.SIADAP2_WITH_QUOTAS,
				SiadapUniverse.SIADAP2, getConfiguration()
						.getQuotaExcellencySiadap2WithQuota().intValue(),
				getConfiguration().getQuotaRelevantSiadap2WithQuota()
						.intValue(), universeDisplayMode, siadap2WithQuotas,
				validationSiadap2WithQuotas.keySet().iterator().next()));

		universeWrappers.add(new SiadapUniverseWrapper(
				validationSiadap2WithoutQuotas.values().iterator().next(),
				SiadapUniverseWrapper.SIADAP2_WITHOUT_QUOTAS,
				SiadapUniverse.SIADAP2, getConfiguration()
						.getQuotaExcellencySiadap2WithoutQuota(),
				getConfiguration().getQuotaRelevantSiadap2WithoutQuota(),
				universeDisplayMode, siadap2WithoutQuotas,
				validationSiadap2WithoutQuotas.keySet().iterator().next()));

		universeWrappers.add(new SiadapUniverseWrapper(
				validationSiadap3WithQuotas.values().iterator().next(),
				SiadapUniverseWrapper.SIADAP3_WITH_QUOTAS,
				SiadapUniverse.SIADAP3, getConfiguration()
						.getQuotaExcellencySiadap3WithQuota(),
				getConfiguration().getQuotaRelevantSiadap3WithQuota(),
				universeDisplayMode, siadap3WithQuotas,
				validationSiadap3WithQuotas.keySet().iterator().next()));

		universeWrappers.add(new SiadapUniverseWrapper(
				validationSiadap3WithoutQuotas.values().iterator().next(),
				SiadapUniverseWrapper.SIADAP3_WITHOUT_QUOTAS,
				SiadapUniverse.SIADAP3, getConfiguration()
						.getQuotaExcellencySiadap3WithoutQuota(),
				getConfiguration().getQuotaRelevantSiadap3WithoutQuota(),
				universeDisplayMode, siadap3WithoutQuotas,
				validationSiadap3WithoutQuotas.keySet().iterator().next()));
	...
	public static boolean isValidSIADAPHarmonizationUnit(Unit unit, int year) {
		...
		if (unitSiadapWrapper.isHarmonizationUnit() && unitSiadapWrapper.isConnectedToTopUnit(harmonizationUnitRelation))
			return true;
		else return isValidSIADAPHarmonizationUnit(unitSiadapWrapper.getSuperiorHarmonizationUnit(), year);
	}
	 ...
	 * @param year
	 * @return true if the given unit is in the hierarchical model (using the siadap unit relations) of the SIADAP for the given year
	 */
	public static boolean isValidSIADAPUnit(Unit unit, int year) {
		AccountabilityType unitRelations = SiadapYearConfiguration
				.getSiadapYearConfiguration(year).getUnitRelations();
		...
		if (children.size() == 0 && parents.size() == 0)
			return false;
		else return new UnitSiadapWrapper(unit, year).isConnectedToTopUnit(unitRelations);
	...
	public boolean isConnectedToTopUnit(AccountabilityType accountabilityType) {
		Unit siadapStructureTopUnit = getConfiguration()
				.getSiadapStructureTopUnit();
		return getUnit().getAncestors(accountabilityType).contains(
				siadapStructureTopUnit);
	...
	protected Map<Integer, Collection<PersonSiadapWrapper>> getValidationPersonSiadapWrappers(
		...
		getUnitAttachedPersons(getUnit(), listPeopleToUse, true,
				new Predicate() {
					...
					public boolean evaluate(Object arg0) {
						...
						if (wrapper.isQuotaAware() != belongsToInstitutionalQuota)
							return false;
						SiadapEvaluationUniverse siadapEvaluationUniverseForSiadapUniverse = wrapper
								.getSiadap()
								.getSiadapEvaluationUniverseForSiadapUniverse(
										universe);
				...
				}, Collections.singleton(getConfiguration()
						.getHarmonizationUnitRelations()), universe
	...
	public String getTotalPeopleWorkingInUnitDescriptionString(
			boolean continueToSubUnits) {
		Integer peopleWithQuotas = getTotalPeopleWorkingInUnit(continueToSubUnits);
		int peopleWithNoQuotas = getTotalPeopleWorkingInUnit(getUnit(),
				continueToSubUnits, getConfiguration()
						.getWorkingRelationWithNoQuota());

		return BundleUtil.getFormattedStringFromResourceBundle(
				"resources/SiadapResources",
				"label.totalWorkingPeopleInUnit.description",
				String.valueOf(peopleWithQuotas + peopleWithNoQuotas),
				String.valueOf(peopleWithQuotas),
				String.valueOf(peopleWithNoQuotas));
	}
	...
	public int getTotalPeopleHarmonizedInUnitWithSiadapStarted() {
		...
		SiadapYearConfiguration siadapYearConfiguration = SiadapYearConfiguration
				.getSiadapYearConfiguration(getYear());

		boolean excludeResponsibles;
		if (!siadapYearConfiguration.getSiadapStructureTopUnit().equals(
				getUnit()))
			excludeResponsibles = true;
		else
			excludeResponsibles = false;

		AccountabilityType siadap2HarmonizationRelation = siadapYearConfiguration
				.getSiadap2HarmonizationRelation();
		AccountabilityType siadap3HarmonizationRelation = siadapYearConfiguration
				.getSiadap3HarmonizationRelation();
		getUnitAttachedPersons(getUnit(), personSiadapWrappers,
				isHarmonizationUnit(), new SiadapStateFilter(
						excludeResponsibles),
				Collections.singleton(getConfiguration()
	...
	public int getTotalPeopleWorkingInUnitIncludingNoQuotaPeople(
		...
		return getTotalPeopleWorkingInUnit(getUnit(), continueToSubUnits,
				configuration.getWorkingRelation(),
	...
	private int getTotalPeopleWorkingInUnit(Unit unit,
			boolean continueToSubUnit, AccountabilityType... workingRelations) {
		int people = 0;
		UnitSiadapWrapper wrapper = new UnitSiadapWrapper(unit, getYear());
		Collection<Person> childPersons = wrapper
				.getChildPersons(workingRelations);
		people += childPersons.size();
		if (continueToSubUnit) {
			for (Unit subUnit : wrapper.getChildUnits(getConfiguration()
					.getUnitRelations())) {
				people += getTotalPeopleWorkingInUnit(subUnit,
						continueToSubUnit, workingRelations);
			}
		}
		return people;
	}

	public int getQuotaAwareTotalPeopleWorkingInUnit(
			boolean continueToSubUnits, boolean withQuota) {
		return withQuota ? getTotalPeopleWorkingInUnit(getUnit(),
				continueToSubUnits, getConfiguration().getWorkingRelation())
				: getTotalPeopleWorkingInUnit(getUnit(), continueToSubUnits,
	...
	public int getTotalPeopleWithSiadapWorkingInUnit(boolean continueToSubUnits) {
		return getTotalPeopleWithSiadapWorkingInUnit(getUnit(),
				continueToSubUnits);
	}

	private int getTotalPeopleWithSiadapWorkingInUnit(Unit unit,
			boolean continueToSubUnit) {
		int people = 0;
		int year = getYear();
		Collection<Person> childPersons = unit
				.getChildPersons(getConfiguration().getWorkingRelation());
		...
		if (continueToSubUnit) {
			for (Unit subUnit : unit.getChildUnits(getConfiguration()
					.getUnitRelations())) {
				people += getTotalPeopleWithSiadapWorkingInUnit(subUnit,
						continueToSubUnit);
			}
		}
		return people;
	...
	private int getNrEvaluationsBasedOnPredicate(
			Collection<PersonSiadapWrapper> personsToEvaluatePredicateOn,
			Predicate predicate) {
		int counter = 0;
	...
	public ArrayList<SiadapException> executeValidation(
		...
		// is the top unit
		if (validationSubActivity
				.equals(ValidationSubActivity.TERMINATE_VALIDATION)) {
			if (!getUnit().equals(
					getConfiguration().getSiadapStructureTopUnit())) {
				throw new ValidationTerminationException(
						"error.validation.must.be.closed.on.top.unit.only");
			}
			...
			for (SiadapUniverseWrapper siadapUniverseWrapper : siadapUniverseWrappers) {
				if (siadapUniverseWrapper.isAboveQuotasValidation()) {
					String isQuotaAware = siadapUniverseWrapper
							.getSiadapUniverse().iterator().next()
							.isQuotaAware() ? "Sim" : "Não";
					throw new ValidationTerminationException(
							"error.validation.above.quotas",
							siadapUniverseWrapper.getSiadapUniverseEnum()
									.getLocalizedName(), isQuotaAware);
				}

			}
		}

		WorkflowActivity activity = SiadapProcess
				.getActivityStaticly(Validation.class.getSimpleName());

		for (SiadapUniverseWrapper universeWrapper : siadapUniverseWrappers) {
			for (PersonSiadapWrapper personSiadapWrapper : universeWrapper
					.getSiadapUniverse()) {
				ActivityInformation<?> validationActivityInformation = new ValidationActivityInformation(
						personSiadapWrapper, activity, validationSubActivity,
				...
				if (validationActivityInformation.hasAllneededInfo()) {
					...
					if (validationSubActivity
						...
						if (personSiadapWrapper.getEvaluator() != null)
							evaluatorsToNotify.add(personSiadapWrapper
									.getEvaluator().getPerson());
					}
				}
			}
		}
		...
		for (Person evaluator : evaluatorsToNotify) {
			try {
				ValidationSubActivity.notifyEvaluatorOfFinishedValidation(
						evaluator, getYear());
	...
	public BigDecimal getExcellencyEvaluationPercentage() {
		...
		Collection<PersonSiadapWrapper> excellentEvaluationPersons = getUnitEmployees(
				true, new Predicate() {
					...
					public boolean evaluate(Object personObject) {
						...
						if (personWrapper.getSiadap() == null
								|| personWrapper.getSiadap()
										.getDefaultSiadapEvaluationUniverse() == null) {
							return false;
						}
						return personWrapper.getSiadap()
		...
		return new BigDecimal(excellentCount).divide(
				new BigDecimal(totalPeopleWorkingForUnit),
				UnitSiadapWrapper.SCALE, RoundingMode.HALF_EVEN).multiply(
				new BigDecimal(100), new MathContext(UnitSiadapWrapper.SCALE));
	}

	public BigDecimal getRelevantEvaluationPercentage() {
		int totalPeopleWorkingForUnit = getUnitEmployees(true).size();
		Collection<PersonSiadapWrapper> relevantEvaluationPersons = getUnitEmployees(
				true, new Predicate() {
		...
		return new BigDecimal(relevantCount).divide(
				new BigDecimal(totalPeopleWorkingForUnit),
				UnitSiadapWrapper.SCALE, RoundingMode.HALF_EVEN).multiply(
				new BigDecimal(100), new MathContext(UnitSiadapWrapper.SCALE));
	}
	...
	public Integer getNumberCurrentExcellentsSiadap2WithQuota() {
		return getNrEvaluationsBasedOnPredicate(
				getSiadap2AndWorkingRelationWithQuotaUniverse(),
				new Predicate() {
					...
					public boolean evaluate(Object arg0) {
						...
						if (siadap != null
								&& siadap.hasGivenSiadapGlobalEvaluation(
	...
	public Integer getRelevantSiadap2WithQuotaQuota() {
		Integer quotaRelevantSiadap2WithQuota = SiadapYearConfiguration
				.getSiadapYearConfiguration(getYear())
	...
	public Integer getExcellencySiadap2WithoutQuotaQuota() {
		Integer quotaExcellencySiadap2WithoutQuota = SiadapYearConfiguration
				.getSiadapYearConfiguration(getYear())
				.getQuotaExcellencySiadap2WithoutQuota();
		int totalPeople = getPeopleHarmonizedInUnitSiadap2WithQuotas();
	...
	public Integer getRelevantSiadap2WithoutQuotaQuota() {
		Integer quotaRelevantSiadap2WithoutQuota = SiadapYearConfiguration
				.getSiadapYearConfiguration(getYear())
	...
	public Integer getExcellencySiadap3WithQuotaQuota() {
		Integer quotaExcellencySiadap3WithQuota = SiadapYearConfiguration
				.getSiadapYearConfiguration(getYear())
				.getQuotaExcellencySiadap3WithQuota();
	...
	public Integer getRelevantSiadap3WithQuotaQuota() {
		Integer quotaRelevantSiadap3WithQuota = SiadapYearConfiguration
				.getSiadapYearConfiguration(getYear())
	...
	public Integer getExcellencySiadap3WithoutQuotaQuota() {
		Integer quotaExcellencySiadap3WithoutQuota = SiadapYearConfiguration
				.getSiadapYearConfiguration(getYear())
				.getQuotaExcellencySiadap3WithoutQuota();
		int totalPeople = getPeopleHarmonizedInUnitSiadap3WithQuotas();
	...
	public Integer getRelevantSiadap3WithoutQuotaQuota() {
		Integer quotaRelevantSiadap3WithoutQuota = SiadapYearConfiguration
				.getSiadapYearConfiguration(getYear())
	...
	private int calculateQuota(int totalPeople, Integer quota) {
		BigDecimal result = new BigDecimal(totalPeople).multiply(
				new BigDecimal(quota)).divide(new BigDecimal(100));
	...
	public boolean isResponsibleForHarmonization() {
		return !getChildPersons(
				getConfiguration().getHarmonizationResponsibleRelation())
				.isEmpty()
				|| (getUnit().equals(
						getConfiguration().getSiadapStructureTopUnit()) && getConfiguration()
						.isCurrentUserMemberOfCCA())
				|| isSpecialHarmonizationUnit()
	...
	private boolean isPersonResponsibleForHarmonization(Unit unit, Person person) {
		Collection<Person> childPersons = getChildPersons(unit,
				getConfiguration().getHarmonizationResponsibleRelation());
		if (childPersons.contains(person)) {
			return true;
		} else {
			Collection<Unit> parentUnits = getParentUnits(unit,
					getConfiguration().getHarmonizationUnitRelations());
			if (parentUnits.isEmpty()) {
				return false;
			} else {
				return isPersonResponsibleForHarmonization(parentUnits
	...
	public void addResponsibleForHarmonization(Person person) {
		AccountabilityType harmonizationResponsibleRelation = getConfiguration()
				.getHarmonizationResponsibleRelation();
		Collection<Accountability> childrenAccountabilities = getUnit()
				.getChildrenAccountabilities(
		...
		getUnit().addChild(person, harmonizationResponsibleRelation,
				new LocalDate(), end);
	}

	public boolean isSiadap2WithQuotasAboveQuota() {
		return new SiadapUniverseWrapper(
				getSiadap2AndWorkingRelationWithQuotaUniverse(),
				"siadap2WithQuotas", SiadapUniverse.SIADAP2, getConfiguration()
						.getQuotaExcellencySiadap2WithQuota(),
				getConfiguration().getQuotaRelevantSiadap2WithQuota(), null,
				null, null).isAboveQuotasHarmonization();
	}
	...
	public boolean isSiadap2WithoutQuotasAboveQuota() {
		return new SiadapUniverseWrapper(
				getSiadap2AndWorkingRelationWithoutQuotaUniverse(),
				"siadap2WithoutQuotas", SiadapUniverse.SIADAP2,
				getConfiguration().getQuotaExcellencySiadap2WithoutQuota(),
				getConfiguration().getQuotaRelevantSiadap2WithoutQuota(), null,
				null, null).isAboveQuotasHarmonization();
	}
	...
	public boolean isSiadap3WithQuotasAboveQuota() {
		...
		return new SiadapUniverseWrapper(
				getSiadap3AndWorkingRelationWithQuotaUniverse(),
				"siadap3WithQuotas", SiadapUniverse.SIADAP3, getConfiguration()
						.getQuotaExcellencySiadap3WithQuota(),
				getConfiguration().getQuotaRelevantSiadap3WithQuota(), null,
				null, null).isAboveQuotasHarmonization();
	}
	...
	public boolean isSiadap3WithoutQuotasAboveQuota() {
		return new SiadapUniverseWrapper(
				getSiadap3AndWorkingRelationWithoutQuotaUniverse(),
				"siadap3WithoutQuotas", SiadapUniverse.SIADAP3,
				getConfiguration().getQuotaExcellencySiadap3WithoutQuota(),
				getConfiguration().getQuotaRelevantSiadap3WithoutQuota(), null,
				null, null).isAboveQuotasHarmonization();
	}
	...
	public Unit getSuperiorHarmonizationUnit() {
		Collection<Unit> parentUnits = getParentUnits(getConfiguration()
				.getHarmonizationUnitRelations());
	...
	public List<PersonSiadapWrapper> getUnitEmployeesWithProcessesInReviewCommission() {
		List<PersonSiadapWrapper> unitEmployees = getUnitEmployees(true,
				new Predicate() {
					@Override
					public boolean evaluate(Object personObject) {
						PersonSiadapWrapper personWrapper = (PersonSiadapWrapper) personObject;
						if (personWrapper.getSiadap()
								.isWaitingForReviewCommission()) {
							return true;
						}
						return false;
					}
	...
	public List<PersonSiadapWrapper> getUnitEmployees(
			boolean continueToSubUnits, Predicate predicate) {
		...
		if (isHarmonizationUnit()) {
			...
			unitAccTypesToUse.add(getConfiguration()
					.getHarmonizationUnitRelations());
		} else {
			// let's iterate useing the unit relations at the very least
			unitAccTypesToUse.add(getConfiguration().getUnitRelations());
		}
		getUnitAttachedPersons(getUnit(), employees, continueToSubUnits,
				predicate, unitAccTypesToUse, getConfiguration()
						.getWorkingRelation(), getConfiguration()
						.getWorkingRelationWithNoQuota());
		return employees;
	}

	public List<PersonSiadapWrapper> getUnitEmployeesWithQuotas(
			boolean continueToSubUnits) {
	...
	public List<PersonSiadapWrapper> getUnitEmployeesWithoutQuotas(
			boolean continueToSubUnits) {
	...
	private void getUnitAttachedPersons(Unit unit,
			List<PersonSiadapWrapper> employees, boolean continueToSubunits,
			Predicate predicate,
			Collection<AccountabilityType> unitAccTypesToUse,
			AccountabilityType... accountabilities) {
		...
		if (continueToSubunits) {
			for (Unit subUnit : wrapper.getChildUnits(unitAccTypesToUse
					.toArray(new AccountabilityType[0]))) {
				getUnitAttachedPersons(subUnit, employees, continueToSubunits,
						predicate, unitAccTypesToUse, accountabilities);
			}
		}
	}
	...
	class SiadapUniverseFilter implements Predicate {
		 ...
		 */
		public SiadapUniverseFilter(boolean excludeResponsibles, int year,
				UnitSiadapWrapper unit, boolean includeQuota,
				Boolean includePositivelyHarmonizedOnly,
		...
		public SiadapUniverseFilter(boolean excludeResponsibles, int year,
			...
			this.harmonizationResponsibles = unit
					.getHarmonizationResponsibles();
		}
		...
		public boolean evaluate(Object arg0) {
			...
			return (personWrapper.getYear() == year
					&& (!excludeResponsibles || !harmonizationResponsibles
							.contains(personWrapper.getPerson()))
					&& (includeQuota && quotaAware || !includeQuota
							&& !quotaAware) && (checkOnHarmonizationAssessment(personWrapper)));
		}
		 ...
		 * @return true if
		 */
		private boolean checkOnHarmonizationAssessment(
				PersonSiadapWrapper personWrapper) {
			if (includePositivelyHarmonizedOnly != null) {
				...
				case SIADAP2:
					return ((includePositivelyHarmonizedOnly && ((personWrapper
							.getHarmonizationCurrentAssessmentForSIADAP2() != null && personWrapper
							.getHarmonizationCurrentAssessmentForSIADAP2()
							.booleanValue()) || (!personWrapper
							.getWithoutExcellencyAwardForSiadap2()
							&& personWrapper
									.getHarmonizationCurrentAssessmentForExcellencyAwardForSIADAP2() != null && personWrapper
							...
							&& personWrapper
									.getHarmonizationCurrentAssessmentForExcellencyAwardForSIADAP3() != null && !personWrapper
								.getHarmonizationCurrentAssessmentForExcellencyAwardForSIADAP3())));
				case SIADAP3:
					return ((includePositivelyHarmonizedOnly && ((personWrapper
							.getHarmonizationCurrentAssessmentForSIADAP3() != null && personWrapper
	...
	public Set<PersonSiadapWrapper> getSiadap2AndWorkingRelationWithQuotaUniverse(
		...
		AccountabilityType siadap2HarmonizationRelation = siadapYearConfiguration
				.getSiadap2HarmonizationRelation();
		boolean excludeResponsibles;
		if (!SiadapYearConfiguration.getSiadapYearConfiguration(getYear())
				.getSiadapStructureTopUnit().equals(getUnit()))
			excludeResponsibles = true;
		else
			excludeResponsibles = false;

		SiadapUniverseFilter siadapUniverseFilter = new SiadapUniverseFilter(
				excludeResponsibles, getYear(), this, true,
		...
		getUnitAttachedPersons(unit, universePersons, isHarmonizationUnit()
				|| isSiadapStructureTopUnit(), siadapUniverseFilter,
				Collections.singleton(getConfiguration()
						.getHarmonizationUnitRelations()),
				siadap2HarmonizationRelation);
	...
	public Set<PersonSiadapWrapper> getSiadap3AndWorkingRelationWithQuotaUniverse(
		...
		AccountabilityType siadap3HarmonizationRelation = siadapYearConfiguration
				.getSiadap3HarmonizationRelation();
		AccountabilityType workingRelation = siadapYearConfiguration
				.getWorkingRelation();
		boolean excludeResponsibles;
		if (!SiadapYearConfiguration.getSiadapYearConfiguration(getYear())
				.getSiadapStructureTopUnit().equals(getUnit()))
	...
	public Set<PersonSiadapWrapper> getSiadap2AndWorkingRelationWithoutQuotaUniverse(
		...
		SiadapUniverseFilter siadapUniverseFilter = new SiadapUniverseFilter(
				excludeResponsibles, getYear(), this, false,
				includePositivelyHarmonizedOnly, SiadapUniverse.SIADAP2);
	...
	public List<UnitSiadapWrapper> getSubHarmonizationUnits() {
		...
		for (Unit unit : this.getChildUnits(getConfiguration()
				.getHarmonizationUnitRelations())) {
	...
	private void fillAllChildUnits(UnitSiadapWrapper wrapper,
			SiadapYearConfiguration configuration,
			List<UnitSiadapWrapper> wrappers, AccountabilityType accToUse) {
		int year = configuration.getYear();

		for (Unit unit : wrapper.getChildUnits(accToUse)) {
			UnitSiadapWrapper unitSiadapWrapper = new UnitSiadapWrapper(unit,
					year);
			wrappers.add(unitSiadapWrapper);
	...
	public UnitSiadapWrapper getTopHarmonizationUnit() {
		SiadapYearConfiguration configuration = getConfiguration();
		AccountabilityType harmonizationUnitRelation = configuration
				.getHarmonizationUnitRelations();
		...
		if (parentUnits.isEmpty()) {
			...
			int year = configuration.getYear();
			UnitSiadapWrapper topUnit = new UnitSiadapWrapper(parentUnits
					.iterator().next(), year);
	...
	public boolean isHarmonizationFinished() {
		...
		return getConfiguration().getHarmonizationClosedUnits().contains(
				unitToUse);
	}

	public boolean isWithAllSiadapsFilled(boolean checkForUsersWithoutSiadap) {
		for (Person person : getChildPersons(getConfiguration()
				.getWorkingRelation(), getConfiguration()
				.getWorkingRelationWithNoQuota())) {
			Siadap siadap = new PersonSiadapWrapper(person, getYear())
					.getSiadap();
			if ((siadap == null && checkForUsersWithoutSiadap)
					|| (siadap != null && !siadap.isDefaultEvaluationDone())) {
				return false;
			}
		}
		return true;
	...
	public Interval getHarmonizationInterval() {
		SiadapYearConfiguration configuration = getConfiguration();
		LocalDate harmonizationBegin = configuration
				.getFirstLevelHarmonizationBegin();
		LocalDate harmonizationEnd = configuration
				.getFirstLevelHarmonizationEnd();
		return new Interval(
				SiadapMiscUtilClass.convertDateToBeginOfDay(harmonizationBegin),
	...
	public static boolean isHarmonizationPeriodOpen(
			SiadapYearConfiguration configuration) {
		if (configuration == null
				|| configuration.getFirstLevelHarmonizationBegin() == null)
			return false;

		Interval interval = new Interval(
				SiadapMiscUtilClass.convertDateToBeginOfDay(configuration
						.getFirstLevelHarmonizationBegin()),
				SiadapMiscUtilClass.convertDateToEndOfDay(configuration
						.getFirstLevelHarmonizationEnd()));
	...
	public void finishHarmonization() {
		if (!isHarmonizationUnit()) {
			throw new SiadapException(
					"error.cannot.close.harmonization.in.a.non-harmonization.unit");
		}
		LocalDate currentDate = new LocalDate();
		SiadapYearConfiguration configuration = getConfiguration();
		if (!isHarmonizationActive())
			throw new SiadapException("error.harmonization.period.is.closed");
		if (configuration.getLockHarmonizationOnQuota()
				&& (isSiadap2WithQuotasAboveQuota() || isSiadap3WithQuotasAboveQuota())) {
			throw new SiadapException(
					"error.harmonization.unit.is.not.harmonized.for.quota");
		}
		if (configuration.getLockHarmonizationOnQuotaOutsideOfQuotaUniverses()
			...
			throw new SiadapException(
					"error.harmonization.unit.is.not.harmonized.outside.quotas.universes");
		}
	...
	public void finishHarmonizationFor(SiadapUniverse siadapUniverse,
		...
		if (siadapUniverse.equals(SiadapUniverse.SIADAP2)) {
			personsToHarmonize
					.addAll(getSiadap2AndWorkingRelationWithQuotaUniverse());
			personsToHarmonize
					.addAll(getSiadap2AndWorkingRelationWithoutQuotaUniverse());
		} else if (siadapUniverse.equals(SiadapUniverse.SIADAP3)) {
			personsToHarmonize
					.addAll(getSiadap3AndWorkingRelationWithoutQuotaUniverse());
			personsToHarmonize
					.addAll(getSiadap3AndWorkingRelationWithQuotaUniverse());
		}
		for (PersonSiadapWrapper personToHarmonize : personsToHarmonize) {
			personToHarmonize.getSiadap().markAsHarmonized(currentDate,
					siadapUniverse);
		}
	}
	...
	public void reOpenHarmonizationFor(SiadapUniverse siadapUniverse) {
		...
		for (PersonSiadapWrapper personToHarmonize : personsToHarmonize) {
			personToHarmonize.getSiadap().removeHarmonizationMark(
					siadapUniverse);
		}
	...
	public void reOpenHarmonization() {
		...
		if (!isHarmonizationFinished())
			throw new SiadapException(
					"error.harmonization.can.not.reopen.unclosed.harmonization");
	...
	public List<PersonSiadapWrapper> getPeopleHarmonizedWithAnyNoAssessment() {
		...
		getUnitAttachedPersons(
				...
				new Predicate() {
					...
					public boolean evaluate(Object arg0) {
						PersonSiadapWrapper personWrapper = (PersonSiadapWrapper) arg0;
						for (SiadapEvaluationUniverse evalUniverse : personWrapper
								.getSiadap().getSiadapEvaluationUniverses()) {
							if (evalUniverse.getHarmonizationAssessment() != null
									&& evalUniverse
											.getHarmonizationAssessment() == false)
								return true;
							if (evalUniverse
									.getHarmonizationAssessmentForExcellencyAward() != null
									&& evalUniverse
											.getHarmonizationAssessmentForExcellencyAward() == false)
								return true;
						}
						return false;
					}
				}, Collections.singleton(siadapYearConf
						.getHarmonizationUnitRelations()),
				siadapYearConf.getSiadap2HarmonizationRelation(),
				siadapYearConf.getSiadap3HarmonizationRelation());
