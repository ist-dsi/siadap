...
public class UnitSiadapWrapper extends PartyWrapper implements Serializable {
    ...
    public Collection<SiadapUniverseWrapper> getValidationUniverseWrappers() {
        ...
        universeWrappers.add(new SiadapUniverseWrapper(validationSiadap2WithQuotas.values().iterator().next(),
                SiadapUniverseWrapper.SIADAP2_WITH_QUOTAS, SiadapUniverse.SIADAP2, getConfiguration()
                        .getQuotaExcellencySiadap2WithQuota().intValue(), getConfiguration()
                        .getQuotaRelevantSiadap2WithQuota().intValue(), universeDisplayMode, siadap2WithQuotas,
        ...
        universeWrappers.add(new SiadapUniverseWrapper(validationSiadap2WithoutQuotas.values().iterator().next(),
                SiadapUniverseWrapper.SIADAP2_WITHOUT_QUOTAS, SiadapUniverse.SIADAP2, getConfiguration()
                        .getQuotaExcellencySiadap2WithoutQuota(), getConfiguration()
                        .getQuotaRelevantSiadap2WithoutQuota(), universeDisplayMode, siadap2WithoutQuotas,
        ...
        universeWrappers.add(new SiadapUniverseWrapper(validationSiadap3WithoutQuotas.values().iterator().next(),
                SiadapUniverseWrapper.SIADAP3_WITHOUT_QUOTAS, SiadapUniverse.SIADAP3, getConfiguration()
                        .getQuotaExcellencySiadap3WithoutQuota(), getConfiguration()
                        .getQuotaRelevantSiadap3WithoutQuota(), universeDisplayMode, siadap3WithoutQuotas,
    ...
    public static boolean isValidSIADAPHarmonizationUnit(Unit unit, int year) {
        ...
        if (unitSiadapWrapper.isHarmonizationUnit()
                && unitSiadapWrapper.isConnectedToTopUnit(harmonizationUnitRelation))
            return true;
        else
    ...
    public int getTotalPeopleHarmonizedInUnitWithSiadapStarted() {
        ...
        getUnitAttachedPersons(getUnit(), personSiadapWrappers, isHarmonizationUnit(), new SiadapStateFilter(
                excludeResponsibles), Collections.singleton(getConfiguration().getHarmonizationUnitRelations()),
    ...
    private int getTotalPeopleWorkingInUnit(Unit unit, boolean continueToSubUnit,
            AccountabilityType... workingRelations) {
        int people = 0;
    ...
    public int getQuotaAwareTotalPeopleWorkingInUnit(boolean continueToSubUnits, boolean withQuota) {
        return withQuota ? getTotalPeopleWorkingInUnit(getUnit(), continueToSubUnits, getConfiguration()
                .getWorkingRelation()) : getTotalPeopleWorkingInUnit(getUnit(), continueToSubUnits, getConfiguration()
                .getWorkingRelationWithNoQuota());
    }
        ...
        if (validationSubActivity.equals(ValidationSubActivity.TERMINATE_VALIDATION)) {
            ...
            for (SiadapUniverseWrapper siadapUniverseWrapper : siadapUniverseWrappers) {
                if (siadapUniverseWrapper.isAboveQuotasValidation()) {
                    String isQuotaAware = siadapUniverseWrapper.getSiadapUniverse().iterator().next().isQuotaAware() ? "Sim"
                            : "Não";
        ...
        for (SiadapUniverseWrapper universeWrapper : siadapUniverseWrappers) {
            for (PersonSiadapWrapper personSiadapWrapper : universeWrapper.getSiadapUniverse()) {
                ActivityInformation<?> validationActivityInformation = new ValidationActivityInformation(
                        personSiadapWrapper, activity, validationSubActivity, universeWrapper.getSiadapUniverseEnum());
    ...
    public BigDecimal getExcellencyEvaluationPercentage() {
        ...
        return new BigDecimal(excellentCount).divide(new BigDecimal(totalPeopleWorkingForUnit),
                UnitSiadapWrapper.SCALE, RoundingMode.HALF_EVEN).multiply(new BigDecimal(100),
                new MathContext(UnitSiadapWrapper.SCALE));
    }
    ...
    public Integer getNumberCurrentExcellentsSiadap2WithQuota() {
        return getNrEvaluationsBasedOnPredicate(getSiadap2AndWorkingRelationWithQuotaUniverse(), new Predicate() {
            ...
            public boolean evaluate(Object arg0) {
                ...
                if (siadap != null
                        && siadap.hasGivenSiadapGlobalEvaluation(SiadapGlobalEvaluation.EXCELLENCY,
    ...
    public static Unit createSiadapHarmonizationUnit(int year, MultiLanguageString name, int number) {
        ...
        return siadapStructureTopUnit.create(siadapStructureTopUnit, name,
                HARMONIZATION_UNIT_NAME_PREFIX + String.valueOf(number), harmonizationPartyType,
                harmonizationUnitRelation, beginDate, null);
    }
        ...
        for (Party party : harmonizationPartyType.getParties()) {
            if (!(party instanceof Unit)) {
                throw new SiadapException(
                        "Error, the harmonization party type should only have units associated with it");
            }
    ...
    private boolean isPersonResponsibleForHarmonization(Unit unit, Person person) {
        Collection<Person> childPersons = getChildPersons(unit, getConfiguration()
    ...
    private void getUnitAttachedPersons(Unit unit, List<PersonSiadapWrapper> employees, boolean continueToSubunits,
            Predicate predicate, Collection<AccountabilityType> unitAccTypesToUse,
        ...
        if (continueToSubunits) {
            for (Unit subUnit : wrapper.getChildUnits(unitAccTypesToUse.toArray(new AccountabilityType[0]))) {
                getUnitAttachedPersons(subUnit, employees, continueToSubunits, predicate, unitAccTypesToUse,
                        accountabilities);
            }
        }
    }
    ...
    class SiadapUniverseFilter implements Predicate {
         ...
         */
        public SiadapUniverseFilter(boolean excludeResponsibles, int year, UnitSiadapWrapper unit,
                boolean includeQuota, Boolean includePositivelyHarmonizedOnly, SiadapUniverse siadapUniverse) {
    ...
    public Set<PersonSiadapWrapper> getSiadap2AndWorkingRelationWithQuotaUniverse(
        ...
        AccountabilityType siadap2HarmonizationRelation = siadapYearConfiguration.getSiadap2HarmonizationRelation();
        boolean excludeResponsibles;
        if (!SiadapYearConfiguration.getSiadapYearConfiguration(getYear()).getSiadapStructureTopUnit()
                .equals(getUnit()))
            excludeResponsibles = true;
        else
            excludeResponsibles = false;

        SiadapUniverseFilter siadapUniverseFilter = new SiadapUniverseFilter(excludeResponsibles, getYear(), this,
                true, includePositivelyHarmonizedOnly, SiadapUniverse.SIADAP2);
        List<PersonSiadapWrapper> universePersons = new ArrayList<PersonSiadapWrapper>();
        getUnitAttachedPersons(unit, universePersons, isHarmonizationUnit() || isSiadapStructureTopUnit(),
                siadapUniverseFilter, Collections.singleton(getConfiguration().getHarmonizationUnitRelations()),
    ...
    public Set<PersonSiadapWrapper> getSiadap3AndWorkingRelationWithQuotaUniverse(
        ...
        SiadapUniverseFilter siadapUniverseFilter = new SiadapUniverseFilter(excludeResponsibles, getYear(), this,
                true, includePositivelyHarmonizedOnly, SiadapUniverse.SIADAP3);
    ...
    public Set<PersonSiadapWrapper> getSiadap2AndWorkingRelationWithoutQuotaUniverse(
        ...
        SiadapUniverseFilter siadapUniverseFilter = new SiadapUniverseFilter(excludeResponsibles, getYear(), this,
                false, includePositivelyHarmonizedOnly, SiadapUniverse.SIADAP2);
    ...
    public Set<PersonSiadapWrapper> getSiadap3AndWorkingRelationWithoutQuotaUniverse(
        ...
        SiadapUniverseFilter siadapUniverseFilter = new SiadapUniverseFilter(excludeResponsibles, getYear(), this,
                false, includePositivelyHarmonizedOnly, SiadapUniverse.SIADAP3);
    ...
    public static class UnitTransverseUtil {
         ...
         * @param unit
         * @param configuration
         * @return the active children units, that have active workers, using
         *         {@link SiadapYearConfiguration#getUnitRelations()} for the
         *         units and
        ...
        public static Collection<Party> getActiveChildren(final Unit unit, final SiadapYearConfiguration configuration) {
            return getActiveChildren(unit, configuration, configuration.getUnitRelations(),
         ...
         *            for the last arg of
         *            {@link #hasSomeWorker(Unit, SiadapYearConfiguration, LocalDate, AccountabilityType[])}
         ...
         *         the configuration year, and of the types
         *         {@link SiadapYearConfiguration#getWorkingRelation()} and
         *         {@link SiadapYearConfiguration#getWorkingRelationWithNoQuota()}
         */
        ...
        private static boolean hasSomeWorker(final Unit unit, final SiadapYearConfiguration configuration) {
            for (final Accountability accountability : unit.getChildAccountabilitiesSet()) {
                if (isActiveWorker(accountability, configuration)
                        || (isActiveUnit(accountability, configuration) && hasSomeWorker(
                                (Unit) accountability.getChild(), configuration))) {
                    return true;
                }
            }
            return false;
    ...
    public static Unit getUnit(final OrganizationalModel organizationalModel, String unitExternalId) {
        ...
        return unit == null ? (organizationalModel.hasAnyParties() ? (Unit) organizationalModel.getPartiesIterator()
                .next() : null) : unit;
    }
     ...
     */
    @SuppressWarnings("boxing")
    public static void removeHarmonizationUnitRelation(Unit harmonizationUnit, Unit subUnit, int year,
            String justification) throws SiadapException {
        if (!harmonizationUnit.getPartyTypes().contains(PartyType.readBy(SIADAP_HARMONIZATION_UNIT_TYPE))) {
            throw new SiadapException("given harmonizationUnit: " + harmonizationUnit.getPresentationName()
                    + " must have a PartyType of: " + SIADAP_HARMONIZATION_UNIT_TYPE);
        }
        ...
        if (currentHU != null) {
            ...
            Collection<Accountability> parentAccountabilities = subUnit.getParentAccountabilities(
    ...
    public void connectToTopHarmonizationUnit(String justification) throws IllegalArgumentException, SiadapException {
        ...
        if (!isHarmonizationUnit()) {
            throw new IllegalArgumentException(
                    "this method should only be called in a HarmonizationUnit. Called in unit: "
        ...
        if (isConnectedToTopUnit(configuration.getHarmonizationUnitRelations()))
            ...
            configuration.getSiadapStructureTopUnit().addChild(getUnit(),
                    configuration.getHarmonizationUnitRelations(), configuration.getFirstDay(),
                    configuration.getLastDay(), justification);
        }
    ...
    public boolean deactivateHarmonizationUnit(String justification) throws IllegalArgumentException, SiadapException {
        ...
        // let's disconnect it
        Collection<Accountability> parentAccountabilities = getUnit().getParentAccountabilities(
                configuration.getLastDayForAccountabilities(), configuration.getLastDayForAccountabilities(),
                harmonizationUnitRelation);
        if (parentAccountabilities.size() > 1)
            throw new SiadapException(
                    "Too many accoutabilities for a given day. Data inconsistency on an Harm. Unit relation between unit "
