...
public class SiadapAccountabilityCreationScript extends WriteCustomTask {
    ...
    protected void doService() {
        ...
        AccountabilityTypeBean accountabilityTypeBean =
                new AccountabilityTypeBean("unitRelations", new MultiLanguageString().with(Language.pt,
                        "Relações de unidades SIADAP").with(Language.en, "SIADAP unit relations"));
        ...
        accountabilityTypeBean =
                new AccountabilityTypeBean("harmonizationResponsibleRelation", new MultiLanguageString().with(Language.pt,
                        "Responsáveis por unidades de harmonização").with(Language.en, "Unit harmonization responsibles"));
        AccountabilityType.create(accountabilityTypeBean);
        ...
        accountabilityTypeBean =
                new AccountabilityTypeBean("workingRelation", new MultiLanguageString().with(Language.pt,
        ...
        accountabilityTypeBean =
                new AccountabilityTypeBean("workingRelationWithNoQuota", new MultiLanguageString().with(Language.pt,
                        "Relação de trabalho sem quota").with(Language.en, "Working relation without quota"));
        AccountabilityType.create(accountabilityTypeBean);

        // relacao entre 2 Person e denota que uma Person e responsavel por
        // avaliar a outra (parent = evaluator, child = evaluated)
        accountabilityTypeBean =
                new AccountabilityTypeBean("evaluationRelation", new MultiLanguageString().with(Language.pt,
