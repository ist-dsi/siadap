...
public class Siadap extends Siadap_Base {
	...
	public List<ObjectiveEvaluation> getObjectiveEvaluations() {
		return getEvaluations(ObjectiveEvaluation.class, new Predicate() {
			...
			public boolean evaluate(Object arg0) {
				...
				return objective
						.isValidForVersion(getCurrentObjectiveVersion());
			}
	...
	private <T extends SiadapEvaluationItem> List<T> getEvaluations(
			Class<T> clazz, Predicate predicate) {
		...
		Collections.sort(evaluationItems,
				SiadapEvaluationItem.COMPARATOR_BY_DATE);
	...
	private BigDecimal getEvaluationScoring(
			List<? extends SiadapEvaluationItem> evaluations) {
		if (!isEvaluationDone()) {
		...
		for (SiadapEvaluationItem evaluation : evaluations) {
			...
			if (itemEvaluation == null) {
				throw new DomainException(
						"error.siadapEvaluation.mustFillAllItems",
						DomainException
		...
		return result.divide(new BigDecimal(evaluations.size()),
				Siadap.PRECISION, Siadap.ROUND_MODE);
	}
	...
	private BigDecimal getPonderationResult(BigDecimal scoring,
			Double usedPercentage) {
		BigDecimal percentage = BigDecimal.valueOf(usedPercentage).divide(
				new BigDecimal(100));
	...
	public BigDecimal getTotalEvaluationScoring() {
		return getPonderatedCompetencesScoring().add(
				getPonderatedObjectivesScoring());
	}
	...
	public CompetenceType getCompetenceType() {
		...
		for (SiadapEvaluationItem siadapEvaluationItem : evaluationItems) {
			if (siadapEvaluationItem instanceof CompetenceEvaluation)
				return ((CompetenceEvaluation) siadapEvaluationItem)
						.getCompetence().getCompetenceType();
		}
		return null;
	}
	...
	{
		return (getEvaluatedOnlyByCompetences() != null && ((getEvaluatedOnlyByCompetences().booleanValue() == false && getCompetences().size() >= MINIMUM_COMPETENCES_WITH_OBJ_EVAL_NUMBER)
				|| (getEvaluatedOnlyByCompetences().booleanValue() == true && getCompetences().size() >= MINIMUM_COMPETENCES_WITHOUT_OBJ_EVAL_NUMBER)));
	...
	public boolean hasRelevantEvaluation() {
		return isEvaluationDone()
				&& SiadapGlobalEvaluation.HIGH
						.accepts(getTotalEvaluationScoring());
	}
